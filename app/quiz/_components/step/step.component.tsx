...
interface StepProps {
  ...
  const Step = ({ pergunta, onAnswer, loading }: StepProps) => {
    ...
    return (
      <div className="w-full min-h-screen flex justify-center items-center flex-col gap-8 bg-white rounded-lg">
        ...
        <ul className="w-full">
          {pergunta.Alternativas && pergunta.Alternativas.map((alt: Alternativas, index) => (
            <li className="w-full py-2 px-16" key={alt.id_alternativa} >
              <Button className="w-full py-2 hover:bg-gray-200 transition-all ease-in-out rounded-md text-lg flex gap-4" disabled={loading} handleClick={() => handleAnswerClick(alt.id_alternativa, alt.alternativa_correta)}>
                <p>{index + 1}.</p>
