...
const Button = ({ children, handleClick, disabled, className = '', backgroundColor, textColor, ...props }: ButtonProps) => {
  ...
  return (
    <button
      ...
      className={`px-4 py-2 text-${textColor} rounded-md ${disabled ? 'bg-gray-600 text-gray-400 cursor-not-allowed' : `bg-${backgroundColor} hover:bg-${backgroundColor}`} ${className} ` } // Classes CSS condicionais baseadas em propriedades
